...
Networks are not only independent. They can also interact with each other, and these interactions can be very flexible. They are the basis of most AE2 automation systems. Broadly speaking, as long as two independent ME Networks affect each other, that can be considered network interaction. Using an ME P2P tunnel to transmit channels in [P2P](./p2p_tunnels.md) is one example. Interaction between ME Networks mainly depends on [Interfaces](../items-blocks/interface.md) and [Storage Buses](../items-blocks/storage_bus.md), whose behavior gives different networks different roles.
